...
| — | `ddc-1-03` | **done** (2026-08-29, Opus) — La 22–30 / En 30–43, both extents plate-confirmed at both ends (`CAP./CHAP. IV` opens La 31 / En 44); flag → `verified`. **30 La ¶¶ → 29 En printed paragraphs, ONE MERGE** ({¶4} + {¶5} → `{¶4–5}`) — the only merge in nine pages, otherwise the longest strict-1:1 stretch in the corpus. **Sixteen English notes** (`8 9 1 2 3 4 5 6 7 8 9 1 2 3 4 5`, two full 9→1 wraps) — second-densest chapter after I.ii's seventeen; **I.iv opens at 6.** No Latin apparatus at all. **★★★ Milton argues in open dialogue with a named continental Arminian for the first time in the corpus** — Curcellæus's *Institutio* quoted twice in Latin ([^s9], [^s1]), and Sumner names the **Supralapsarian** doctrine outright ([^s1-2]). **★★★ Milton's own *Artis Logicæ plenior Institutio* quoted THREE times** ([^s2], [^s3], [^s4-2]) — the chapter's causal vocabulary (*per se* / *per accidens*, physical vs. compulsory necessity) is Milton's own logic textbook, not borrowed scholastic Latin. **★★ Twelve of sixteen notes draw on Milton's own corpus** (nine quote his poetry, three his Logic) — the highest concentration of the "defend the doctrine from Milton's own work" pattern yet. **★ `Act. xv. 28` printed twice in Latin where English has `Acts xv. 18` both times** — Acts 15:18 KJV settles it; a doubled, plate-confirmed Latin misprint. **★ Two Jonah 3 citations in one clause both print La one verse high**, one out of range (`v. 11` — Jonah 3 has ten); left `unchecked`, Jonah is not among the 23 read J–T divisions. ⚠ **Caught and fixed in-session: every paragraph label from `{¶4}` onward was first written by sequential count rather than tracked against the Latin ¶ number**, silently masking the merge — corrected before the chunk shipped; see the chunk's own Notes for the standing warning. Two content-filter blocks on the doctrinally dense material, both cleared by writing in smaller pieces. |
| — | `ddc-1-07` | **done** (2026-08-29, Opus) — La 124–139 / En 172–198, both extents plate-confirmed at both ends (`CAP./CHAP. VIII` opens La 140 / En 199); flag → `verified`. **36 La ¶¶ → 36 En + one Sumner-only recap paragraph** (`{¶35 cont.}`, no Latin counterpart), otherwise strict 1:1. **Forty-six English notes — nearly triple the prior record (I.ii's seventeen)** — the 9-cycle repeats five full times, closing on a sixth 1; plus **five Latin notes**, more than any other chapter read so far. **★★★★ Curcellæus's *Institutio* cited EIGHT times** — Milton's two most contested positions here (non-absolute divine action, traducianism) both lean on a named Remonstrant authority. **★★★★ Sumner names and cites TWO rebuttals of Milton's own argument** — Waterland's anti-Arian sermons twice, Beveridge's *Exposition of the First Article* once against the denial of *creatio ex nihilo* — a first in the corpus. **★★★ The Latin apparatus is textual-critical for the first time in Book I**: [^la2] is Sumner's own conjectural repunctuation of a crux he admits he cannot fully construe (matches I.ii's precedent, but here shown working in Latin); [^la3] corrects a single MS word (*iniquam*→*inquam*). **★★★ Twelve notes cite Milton's own prose or poetry as authority** — three Logic self-citations, three Tetrachordon, one each Divorce/Church-Government/Areopagitica, plus ~20 PL and 2 SA passages. **★★ A genuine printer gap**: `[^s7-3]`'s inline anchor (after "the attribute of wings") has no printed definition anywhere on En p. 188 — recorded as a defect, not invented. **★★ A textual-critical note about *Paradise Lost*'s own manuscript** (`[^s6-4]`, the "fowl"/"soul" variant, citing Bentley/Pearce/Newton/Richardson) inside a Genesis footnote. ⚠⚠ **The same sequential-numbering bug caught in ddc-1-03 recurred here at much larger scale** — every paragraph label drifted by a full position before being caught by a mechanical count-and-compare pass; see the chunk's own Notes for the standing warning to run that check after each major section, not only at the end. Several content-filter blocks on the traducianism material, all cleared by writing in smaller pieces. |
| 2+ | `ddc-1-08` … `ddc-1-33` | in order, saving I.iv–I.vi for last (§5) |
