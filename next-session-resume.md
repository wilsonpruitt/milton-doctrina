...
✅ **Deployed 2026-09-25** (CLI-prebuilt; see `metadata/open-layer-launch-checks.md`). The export was NOT refreshed and is still dated 2026-09-18, because refreshing it needs an R2 upload first. The alias index repair is live.
...
✅ **All three owed rulings closed 2026-09-25 (Wilson):** (a) `{¶2 cont.–3}` RATIFIED, now CONVENTIONS §4; (b) the Hebrew points stay **⚠ UNVERIFIED on the web**, and a Hebraist is needed only before a print edition, so don't raise it again for the site; (c) the `ddc-1-09` La ¶1 small-caps fix is APPLIED (first row of the new `metadata/corrections.tsv`, the ledger for any change to published text). ✅ **Opening audit done (2026-09-25):** all 24 chapter openings checked against the plates. I.ix En ¶1 was fixed, and so were 9 more opening lines in I.iii, I.vii, I.viii, II.x, II.xii, II.xiii and II.xiv. CONVENTIONS §2 now has the display-word exception. Every change is in `metadata/corrections.tsv`. ⬜ These fixes are committed but **NOT deployed**. They go out with the next deploy.
...
⛔ **STILL NEEDS WILSON before this chunk can ship — unchanged from the last block.** (a) The combined split-and-merge `{¶2 cont.–3}` wants an explicit dated ruling; §4 has a split form and a merge form but nothing for a continuation that is itself merged forward, and CONVENTIONS forbids drift. Also verify `build-citations.py` recognises the label — an unrecognised merge form silently reports every La ¶ inside it as absent from the English (the I.ix en-dash bug); this label already uses U+2013. (b) The Hebrew wants a **Hebraist** — on the POINTS, for TWO words together: the Exod. 21:10 word (`[^1]`, La 167 / En 237) and `בעל` (`[^2]`, La 162 / En 230), which carry the same unidentified marks. Consonants are secure; the points in the text are an explicit guess. `בעל`’s bare transcription is a PLACEHOLDER — do not cite it as evidence the volumes set it unpointed.

★★ **The Hebrew: `1a5119f`'s reading STANDS — `עְוֹנה`, with a nun.** A later session read the sort as a **bet**, "confirmed" it against the `בעל` control at La 162 / En 230, edited both text layers and rewrote `[^1]`. **Wilson reversed it on sight (2026-09-12): En 237's nun is simply badly struck, and the controls are clean bets.** All of that has been reverted; the file is back to the nun. ⛔ **Keep the lesson, it is cheap and it will recur: a CONTROL ONLY DISCRIMINATES BETWEEN GLYPHS IN COMPARABLE CONDITION.** Matching a damaged sort against a clean one tests whether the damage resembles the control, nothing more — and it produces a confident, evidenced-looking wrong answer. Second guard, free and ignored that time: **the reading yielded a non-word.** ⭐ Letter identity in a script the session does not command belongs to §5c's Hebraist, not to the plate work; the plate work's job is to hand them a legible crop.
...
**(b) The Hebrew at La 167 is SETTLED — only a Hebraist sign-off is still owed.** Resolved off `bwb_T5-ARK-705` (STRUCTURE.md’s cleaner second Latin scan, leaf **n184** = printed 167), which reads at native resolution where our foxed working copy failed at 2400 dpi. Printed form: **`עְוֹנה`** — ayin + shva · vav + holam · nun · he, codepoints checked by Unicode name. It corrected two different errors: a qamats under the nun that had been reconstructed from the standard lexical form (the exact §8b(ii) trap), and a foxing-driven misreading of the last two consonants as gimel/dalet. The pointing is non-standard, so `[^1]` keeps its ⚠ UNVERIFIED mark on the POINTS only; the consonants are secure. ⬜ Check En ~237 for a second witness when the English layer gets there. **Reach for the second scan BEFORE recording any Latin-side reading as unresolvable — STRUCTURE.md says so in terms, and this is now the second time it has paid.**
